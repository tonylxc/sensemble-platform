...
- **上报限流**（每设备最小间隔可配，FR-3.4）+ **设备离线主动告警**（定时巡检 → 站内通知，FR-2.3）
- **Python / JS SDK + 接入示例**（`sdk/`，FR-12.2）
- **MinIO 原始件归档**（打包数据集时尽力归档 CSV，FR-4.3）
- **传感器型号词典 + 数据集元数据版本历史**（FR-5.3）

**🎉 P1 全部完成。** 下一步可进 P2（Kriging 高级插值、DOI/版本、推荐、K8s、CAS、数字孪生 等）。
